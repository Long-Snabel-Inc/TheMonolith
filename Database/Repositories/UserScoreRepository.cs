...
namespace TheMonolith.Database.Repositories
{
    public class UserScoreRepository
    {
        ...
        public async Task Upsert(int source, int target, int score)
        {
            ...
            updateCommand.Parameters.AddWithValue("source", source);
            updateCommand.Parameters.AddWithValue("target", target);
            ...
            if (updateResult < 1)
            {
                ...
                command.Parameters.AddWithValue("source_id", source);
        ...
        public async Task<DateTime> LatestUpdate(int userId)
        {
            await using var connection = await _database.Connection();
